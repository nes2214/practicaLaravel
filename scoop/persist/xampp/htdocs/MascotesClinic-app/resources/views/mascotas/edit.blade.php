...
<div class="min-h-screen bg-gray-50 p-8">
    <div class="max-w-xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-2xl font-semibold text-gray-800 mt-2">Editar registro clínico</h1>
            <p class="text-sm text-gray-500 mt-1">Modifica los datos del registro</p>
        </div>

        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <form action="{{ route('lineas-historial.update', $lineasHistorial) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                    ...
                    <select name="mascota_id"
                        ...
                        @foreach($mascotas as $mascota)
                            <option value="{{ $mascota->id }}"
                                {{ (old('mascota_id', $lineasHistorial->mascota_id) == $mascota->id) ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="fecha"
                           value="{{ old('fecha', $lineasHistorial->fecha->format('Y-m-d')) }}"
                           class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition">
                    @error('fecha')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="motivo_visita"
                           value="{{ old('motivo_visita', $lineasHistorial->motivo_visita) }}"
                ...
                <div>
                    ...
                    <textarea name="descripcion" rows="4"
                              ...
                              class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent transition resize-none">{{ old('descripcion', $lineasHistorial->descripcion) }}</textarea>
                    @error('descripcion')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
